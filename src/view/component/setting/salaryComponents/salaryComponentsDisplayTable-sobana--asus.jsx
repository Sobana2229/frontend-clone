...
function SalaryComponentsDisplayTable({setShowForm, showForm, indexTab}) {
    ...
    const totalRecords = activeTab === "Earnings" 
        ? listEarning?.list?.length || 0 
        : activeTab === "Reimbursements" 
        ? listReimbursement?.list?.length || 0 
        : 0;

    return (
       <div className="w-full flex-1 flex flex-col items-start justify-start bg-white rounded-2xl overflow-hidden" style={{position: 'relative', height: '100%'}}>
            <div className="flex-1 w-full overflow-y-auto" style={{position: 'relative'}}>
                <TableReusable 
                    dataHeaders={
                    ...
                    tableFor={activeTab} 
                    handleEdit={handleActionTable} 
                    handleDelete={handleActionTable} 
                    handleChangeActiveStatus={handleChangeActiveStatus} 
                />
            </div>

       <div className="w-full h-25 px-1 py-2 bg-white border-t border-gray-200 flex items-center justify-end fixed bottom-0 z-10">
  <PaginationPages 
