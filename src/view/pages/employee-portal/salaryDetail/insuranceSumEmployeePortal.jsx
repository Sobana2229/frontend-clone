...
function InsuranceSumEmployeePortal() {
  const {
  ...
  // Helper function untuk menghitung ESI berdasarkan gross wages
  const calculateESIContributions = (grossWages) => {
    const gross = parseFloat(grossWages) || 0;

    // ESI ceiling is ₹21,000 per month
    const esiCeiling = 21000;
    const eligibleWages = Math.min(gross, esiCeiling);

    // Employee: 0.75% of gross wages
    const employeeContribution = Math.round(eligibleWages * 0.0075);

    // Employer: 3.25% of gross wages
    const employerContribution = Math.round(eligibleWages * 0.0325);

    return {
      employee: employeeContribution,
      employer: employerContribution,
      total: employeeContribution + employerContribution,
      eligibleWages: eligibleWages,
    };
  };
  ...
  const getMonthDataByKey = (monthKey) => {
    if (dataEmployeeAnnualEarning && Array.isArray(dataEmployeeAnnualEarning)) {
      const monthItem = dataEmployeeAnnualEarning.find(
        (item) => item[monthKey]
      );
  ...
  // Calculate gross wages dari salary detail
  const calculateGrossWages = (monthData) => {
    ...
    if (monthData.basicMonthly) {
      grossWages += parseFloat(monthData.basicMonthly);
    }
    ...
    allowanceFields.forEach((field) => {
      if (monthData[field]) {
        grossWages += parseFloat(monthData[field]);
      }
    });

    // Add from SalaryDetailComponents
    if (
      monthData.SalaryDetailComponents &&
      Array.isArray(monthData.SalaryDetailComponents)
    ) {
      monthData.SalaryDetailComponents.forEach((component) => {
        if (
          component.type !== "deduction" &&
          parseFloat(component.amount) > 0
        ) {
          grossWages += parseFloat(component.amount);
        }
      });
    }
  ...
  const generateMonthlyContributionData = () => {
    ...
    if (dataEmployeeAnnualEarning && Array.isArray(dataEmployeeAnnualEarning)) {
      monthNames.forEach((monthKey, index) => {
        ...
        if (monthData && Object.keys(monthData).length > 0) {
          const grossWages = calculateGrossWages(monthData);
          const esiContributions = calculateESIContributions(grossWages);

          monthlyData.push({
            month: `${monthLabels[index]} ${actualYear}`,
            grossWages: grossWages,
            yourContribution: esiContributions.employee,
            employerContribution: esiContributions.employer,
            totalContribution: esiContributions.total,
          });
        }
      });
    }
    ...
    if (monthlyData.length === 0) {
      const sampleMonths = [
        { month: `Apr ${actualYear}`, grossWages: 0 },
        { month: `May ${actualYear}`, grossWages: 0 },
        { month: `Jun ${actualYear}`, grossWages: 0 },
        { month: `Jul ${actualYear}`, grossWages: 0 },
      ];

      return sampleMonths.map((item) => {
        const esiContributions = calculateESIContributions(item.grossWages);
        return {
          month: item.month,
  ...
  const calculateTotalsFromMonthly = (monthlyData) => {
    return monthlyData.reduce(
      (acc, item) => ({
        grossWages: acc.grossWages + item.grossWages,
        yourContribution: acc.yourContribution + item.yourContribution,
        employerContribution:
          acc.employerContribution + item.employerContribution,
  ...
  const statutoryDetails = {
    esiNumber: "-", // TODO: ambil dari employee data jika ada
  };
  ...
  const handleDownloadPayslip = async () => {
    setDownloading(true);
    try {
      const epfElement = document.querySelector("[data-insurance-sum]");
      if (!epfElement) {
      ...
      // Filename
      const employeeName =
        dataEmployeePersonalDetail?.Employee?.firstName || "Employee";
      const fileName = `EPF_Contribution_Summary_${selectedYear}_${employeeName.replace(
        /\s+/g,
        "_"
      )}.pdf`;
  ...
  if (loading) {
    return (
      <div className="w-full h-full bg-white flex items-center justify-center">
        <div className="text-gray-500">Loading ESI contribution data...</div>
      </div>
    );
  }
  ...
  const getErrorMessage = (errorObj) => {
    ...
    return "An error occurred while loading ESI contribution data";
  };
    ...
    return (
      <div className="w-full h-full bg-white flex items-center justify-center">
        <div className="text-red-500">
          Error loading ESI contribution data: {getErrorMessage(error)}
        </div>
      </div>
    );
  }
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th
                    scope="col"
                    className="px-6 py-3 text-center text-base font-medium uppercase tracking-wider border-x border-gray-200"
                  >
                    Month
                  </th>
                  <th
            ...
            <div className="text-center py-12">
              <div className="text-gray-500">
                <p className="text-lg font-medium">
                  No ESI contribution data found
                </p>
                <p className="text-sm mt-1">
                  No ESI contribution data available for the selected financial
                  year.
                </p>
              </div>
            </div>
...
export default InsuranceSumEmployeePortal;