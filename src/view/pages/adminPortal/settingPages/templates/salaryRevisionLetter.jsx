...
import { salaryRevisionLetterTemplate } from "../../../../../../data/dummy";
import Modal from "react-modal";
import { useEffect, useState } from "react";
import SalaryRevisionLetter from "../../../../component/pdfTamplates/salaryRevisionLetter/salaryRevisionLetter";
import settingTemplateStoreManagements from "../../../../../store/tdPayroll/setting/templates";
const baseUrl = import.meta.env.VITE_BASEURL;

function SalaryRevisionLetterPages() {
    ...
    return (
        <div className="w-full h-full flex flex-col items-start justify-start space-y-4">
            <HeaderReusable title="Salary Revision Letter" />
        
            <div className="w-[90%] grid grid-cols-4 gap-10 p-5">
                {salaryRevisionLetterTemplate?.map((el, idx) => {
                        ...
                        <div className="w-full h-full space-y-2">
                            ...
                            <div className="w-full h-[300px] rounded-md shadow-md overflow-hidden relative group">
                                <img
                                    ...
                                    src={`/settingTamplates/` + `Salary_Revision_Letter.png`}
                                    alt="thumbnail"
                                />
                                ...
                                <div className="absolute bottom-[-50px] group-hover:bottom-0 left-0 w-full h-12 flex items-center justify-center space-x-4 z-10 bg-black bg-opacity-70 transition-all duration-300 ease-in-out">
                                    ...
                                    {/* <Link to={"/templates/" + `${tamplatesName} Salary Revision Letter-${idx}`} className="p-2 rounded-md bg-white text-black font-medium"><Pencil/></Link> */}
                                </div>
                            </div>
                        </div>
                    )
                ...
                <div className="w-full h-full relative">
                    ...
                    <div className="w-full h-full pt-14">
                        <div className="w-full h-full flex items-start justify-center overflow-y-auto p-4">
                            {currentTamplates === "Standard Template" && ( <SalaryRevisionLetter imgUrl={baseUrl + imgTamplateSetting} availableDetailTamplate={availableDetailTamplate} logoScale={logoScale} />)}
                        </div>
                    </div>
                </div>
            </Modal>
...
export default SalaryRevisionLetterPages;
