...
function LeavesPages() {
  const { fetchZohoLeavesPayroll, dataLeavesPayroll, loading } = zohoStore();
  ...
  useEffect(() => {
    if(dataLeavesPayroll?.length === 0){
      const token = localStorage.getItem("zoho_access_token");
      fetchZohoLeavesPayroll(token, {
  ...
  return (
    <div
      className={`w-full h-screen flex ${
        dataLeavesPayroll?.length < 15 ? "items-start" : "items-center"
        ...
        <div className="w-full">
          <div className="w-full bg-white">
            ...
            <div className="w-full bg-white rounded-lg shadow-sm">
                <TableReusable data={dataLeavesPayroll} tableFor="leavesPages" isTableFull={true} />
            </div>
          </div>
        </div>
      )}
